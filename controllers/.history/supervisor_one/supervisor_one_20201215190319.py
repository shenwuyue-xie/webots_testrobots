...
XPOSITION = {'min':-4, 'max':4}
YPOSITION = {'min':-1.5 , 'max':1.5}
ZPOSITION = {'min': -1, 'max' : 10}
MAX_DISTANCE = {'min':0, 'max':25}
...
class TaskDecisionSupervisor(SupervisorCSV):
    def __init__(self,log_dir,v_action,v_observation,v_reward,windows=[10,100,200]):
    ...
    def get_observations(self):
        self.ds_values = []
        # self.final_distance = [25 for i in range(Max_robotnum)]
        ...
        if len(message) != 0:
            ...
            for k in range(0,self.robot_num):
                ...
                robot_rotation = []
                robot_rotation = self.robot_handles[k].getField('rotation').getSFRotation()
            ...
            target_position = self.final_target.getPosition()
            observation.append(utils.normalize_to_range(float(target_position[0]),XPOSITION['min'],XPOSITION['max'],0,1))
            observation.append(utils.normalize_to_range(float(target_position[1]),YPOSITION['min'],YPOSITION['max'],0,1))
            observation.append(utils.normalize_to_range(float(target_position[2]),ZPOSITION['min'],ZPOSITION['max'],0,1))
    ...
    def get_reward(self,action):
        ...
        if np.min(self.ds_values) <= 50:
            ...
            if self.dscount > 60:
                ...
                for m in range(Max_robotnum):
                    ...
                    reward  =  reward - float(consumption/self.startbattery) * 10
        ...
        if self.final_distance < self.findThreshold:
            ...
            dischange = self.final_distance - self.new_final_distance
            # print(dischange)
            if dischange > 0 :
                reward = reward + 120 * dischange
            else :
                reward = reward + 150 * dischange
            # reward = reward - self.new_final_distance + self.final_distance
    ...
    def respawnRobot(self):
        ...
        self.robot_handles.append(self.supervisor.getFromDef('robot_0'))
...
supervisor_env = TaskDecisionSupervisor(log_dir="logs/results/ddpg", v_action=1,v_observation=1,v_reward=1,windows=[10,\
                                        10000, 2000])
