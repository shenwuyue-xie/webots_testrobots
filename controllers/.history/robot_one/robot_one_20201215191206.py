    ...
    def use_message_data(self,message):
        ...
        for i in range(2):
            if float(message[i]) <0:
                message[i] = self.normalize_to_range(float(message[i]),-0.05,0.1,-8,-4)
            if float(message[i]) >= 0:
                message[i] = self.normalize_to_range(float(message[i]),0.1,1.05,6,12)

        # for j in range(2,14):
        #     # message[i] = float(message[i])
        #     # x = np.random.uniform(0,1,12)
        #     message[j] = self.normalize_to_range(float(message[j]),-0.1,1.1,0,1)
        #     if message [j] >= 0 and message[j] <= 0.3:
        #         message[j] = 0
        #     elif message [j] > 0.4 and message [j] <= 0.7:
        #         message[j] = 0
        #     elif message [j] > 0.8 and message[j] <= 1:
        #         message[j] = 0
        #     elif message[j] > 0.7 and message[j] <= 0.8:
        #         message[j] = self.normalize_to_range(message[j],0.7,0.8,0,math.pi/2)
        #     elif message[j] > 0.3 and message[j] <= 0.4:
        #         message[j] = self.normalize_to_range(message[j],0,0.1,-math.pi/2,0)
