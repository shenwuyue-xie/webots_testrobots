...
class TaskDecisionRobot(RobotEmitterReceiverCSV):
    ...
    def setupmotors(self):
        ...
        self.rightmotor.setVelocity(50)
    ...
    def use_message_data(self,message):
        ...
        for i in range(2):
            if float(message[i]) <0:
                message[i] = self.normalize_to_range(float(message[i]),-0.4,0,-2,-8)
            if float(message[i]) >= 0:
                message[i] = self.normalize_to_range(float(message[i]),0,1.4,8,14)
        ...
        for j in range(2,14):
            ...
            if message [j] >= 0.05 and message[j] <= 0.95:
                message[j] = 0
            elif message[j] > 0.95 and message[i] < 1:
