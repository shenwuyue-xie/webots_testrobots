    ...
    def use_message_data(self,message):
        ...
        for i in range(2):
            if float(message[i]) <0:
                message[i] = self.normalize_to_range(float(message[i]),-0.2,0,-8,-4)
            if float(message[i]) >= 0:
                message[i] = self.normalize_to_range(float(message[i]),0,1.2,8,16)

        for j in range(2,14):
            # message[i] = float(message[i])
            # x = np.random.uniform(0,1,12)
            message[j] = self.normalize_to_range(float(message[j]),-0.1,1.2,0,1)
            if message [j] >= 0.05 and message[j] <= 0.95:
                message[j] = 0
            elif message[j] > 0.95 and message[j] < 1:
                message[j] = self.normalize_to_range(float(message[j]),0.95,1,0,math.pi/2)
            elif message[j] > 0 and message[j] < 0.05:
                message[j] = self.normalize_to_range(float(message[j]),0,0.05,-math.pi/2,0)
