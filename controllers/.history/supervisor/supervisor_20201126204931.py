...
Max_robotnum = 6
OBSERVATION_SPACE =  (Max_robotnum-1) * 4 + 7 + 17 * Max_robotnum +3
ACTION_SPACE = Max_robotnum * 2 + 3
MAX_DSNUM = (Max_robotnum-1) * 4 + 7

DIST_SENSORS_MM = {'min': 0, 'max': 1000}
XPOSITION = {'min':-2.5, 'max':3}
YPOSITION = {'min':-1.5 , 'max':1.3}
ZPOSITION = {'min': -3, 'max' : 4}
MAX_DISTANCE = {'min':0,'max' :}
...
class TaskDecisionSupervisor(SupervisorCSV):
    ...
    def get_observations(self):
        ...
        if len(message) != 0:
            ...
            for k in range(0,self.robot_num):
                ...
                self.distances = []
                self.ditances.append(utils.get_distance_from_target(self.robot_handles[k],self.final_target))
                observation.append(utils.normalize_to_range(float(self.distances[k]),MAX_DISTANCE['min'],MAX_DISTANCE['max']))                
    ...
    def robot_step(self,action,ep):
        ...
        if action[-1] > 0 and action[-1] <= 0.3 and self.robot_num < Max_robotnum:
        ...
        elif action[-1] >0.7 and action[-1] < 1  and self.robot_num >1:
