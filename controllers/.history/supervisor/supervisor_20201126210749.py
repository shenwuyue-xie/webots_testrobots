    ...
    def get_observations(self):
        ...
        if len(message) != 0:
            ...
            for k in range(0,self.robot_num):
                ...
                observation.append(utils.normalize_to_range(float(self.final_distance[k]),MAX_DISTANCE['min'],MAX_DISTANCE['max']))                

                self.angles[k] = utils.get_angle_from_target(self.robot_handles[k],self.final_target)
                observation.append(utils.normalize_to_range(float(self.robot_handles[k]),MAX_ANGLE['min'],MAX_ANGLE['max']))
