...
class ActorNetwork(nn.Module):
    def __init__(self,
        ...
        self.mu_1 = nn.Linear(self.fc3_dims, int(self.n_actions / 2))
        ...
        self.mu_2 = nn.Linear(self.fc6_dims, int(self.n_actions / 2) + 1)
...
class TD3(object):
    def __init__(self,
        ...
        self.noise_1 = OUActionNoise(mu=np.zeros(int(n_actions/2)),
                                   dt=1e-2,
                                   sigma=0.2
                                   # theta=0.15,
                                   )
        self.noise_2 = OUActionNoise(mu=np.zeros(int(n_actions/2)),
                                   dt=1e-2,
                                   sigma=0.15
                                   # theta=0.15,
                                   )
    ...
    def choose_action_train(self, observation):
        if observation is not None:
            ...
            noise_1 = T.tensor(self.noise_1(), dtype=T.float).to(self.actor.device).clamp(-0.4,0.4)
            noise_2 = T.tensor(self.noise_2(), dtype = T.float).to(self.actor.device).clamp(-0.2,0.2)
            # print(noise_1)
            # print(noise_2)
            mu_1,mu_2,mu_3 = T.split(mu,[int(self.n_actions/2),int(self.n_actions/2),1], dim = -1)
            mu_prime_1 = mu_1 + noise_1
            mu_prime_2 = mu_2 + noise_2
            
            mu_prime = T.cat((mu_prime_1,mu_prime_2),dim = -1)
            mu_prime = T.cat((mu_prime,mu_3),dim = -1)
            self.actor.train()
    ...
    def learn(self):
        ...
        with T.no_grad():
            ...
            mu_1,mu_2,mu_3 = T.split(target_actions,[int(self.n_actions/2),int(self.n_actions/2),1], dim = 1)
            ...
            mu_prime = T.cat((mu_prime_1,mu_prime_2),dim = 1)
            # print(mu_prime.size())
            target_actions = T.cat((mu_prime,mu_3),dim = 1)
