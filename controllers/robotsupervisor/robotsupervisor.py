...
from keyboard_printer import KeyboardPrinter
...
class TaskDecisionSupervisor(Supervisor):
    ...
    def get_default_observation(self):
        self.observation = [0 for i in range(self.observation_space)]
...
for i in range(1, 500):
    ...
    score = 0
    supervisor_env.reset()
    supervisor_env.get_default_observation()
    first_iter = True
    if i % 250 == 0:
        print("================= TESTING =================")
        while not done:
            act = agent.choose_action_test(obs).tolist()
            new_state, _, done, _ = supervisor_env.act_step(act)
        ...
        while not done:
            ...
            new_state, reward, done, info = supervisor_env.act_step(act)
